...
function useSearch () {
  ...
  useEffect(() => {
    ...
    if (search === '') {
      setError('Cant search an empty input')
      return
    }

    if (search.match(/^\d+$/)) {
      setError('Cant search a movie with a number')
      return
    }

    if (search.length < 3) {
      setError('Input has to be at least 3 characters')
      return
    }
...
function App() {
  ...
  return (
      ...
      <main>
        { loading ? <p>Loading...</p> : <MovieResults movies={movies} /> }
      </main>
      </div>
  )
}
